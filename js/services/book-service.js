...
function searchBook(bookKey) {
  ...
  return axios
    .get(`https://www.googleapis.com/books/v1/volumes?printType=books&q=${bookKey}`)
    .then((res) => res.data.items.slice(0, 10))
    .then((bookList) => {
      return bookList.map((book) => {
        const { title, subtitle, authors, publishedDate, description, pageCount, categories, language } =
          book.volumeInfo;
        const {} = book.saleInfo;
        var relevant = {
          id: book.id,
    ...
    .catch((err) => console.log('Failed to get data from service', err));
}
